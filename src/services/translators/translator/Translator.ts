...
export interface TranslatorOption {
    ...
    /**
     * 实际的请求方法
     * @param jsonObj 要翻译的 JSON 对象，key 是标识符，value 是要翻译的文本
     * @param fromKey 源语言
     * @param toKey 目标语言
...
export class Translator {
    ...
    async translate(jsonObj: Record<string, string>, fromKey: string, toKey: string) {
        let result: Record<string, string> = {}
        try {
            result = await this.option.fetchMethod(jsonObj, fromKey, toKey)
        } catch (error) {
